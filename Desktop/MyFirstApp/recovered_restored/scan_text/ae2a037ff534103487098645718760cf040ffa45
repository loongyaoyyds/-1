class TodoApp {
    ...
    addCloudSyncButtons() {
        // Add cloud sync buttons to header
        const header = document.querySelector('header');
        const statsDiv = header.querySelector('.stats');

        // Create cloud sync container
        const cloudContainer = document.createElement('div');
        cloudContainer.className = 'cloud-sync-container';
        cloudContainer.innerHTML = `
            <button id="cloud-login" class="btn-cloud">☁️ 登录</button>
            <button id="cloud-sync" class="btn-cloud" style="display: none;">🔄 同步</button>
            <div id="cloud-status" class="cloud-status">离线</div>
        `;

        // Insert before stats
        statsDiv.parentNode.insertBefore(cloudContainer, statsDiv);

        // Bind events
        document.getElementById('cloud-login').addEventListener('click', () => this.showLoginModal());
        document.getElementById('cloud-sync').addEventListener('click', () => this.syncToCloud());
            ...
            <div class="modal-content">
                ...
                <form id="login-form">
                    ...
                    <div class="form-actions">
                        <button type="button" id="register-btn" class="btn-secondary">注册</button>
    ...
    showRegisterModal() {
        ...
        messageDiv.innerHTML = `
            <div style="color: #4a90e2;">
                <h3>注册说明</h3>
                <p>由于这是演示版本，注册功能已禁用。</p>
                <p>您可以直接使用任意邮箱和密码登录来体验云端同步功能。</p>
                <button onclick="document.getElementById('login-message').innerHTML=''" class="btn-primary">确定</button>
            </div>
        `;
    }
    ...
    async handleLogin() {
        ...
        if (!email || !password) {
            this.showLoginMessage('请填写邮箱和密码', 'error');
            return;
        }

        // For demo purposes, we'll simulate a successful login
        // In production, you would use this.supabase.auth.signInWithPassword()
        this.currentUser = {
        ...
        // Save login state
        if (rememberMe) {
            localStorage.setItem('cloud-user', JSON.stringify(this.currentUser));
        }

        // Update UI
        document.getElementById('cloud-login').style.display = 'none';
        document.getElementById('cloud-sync').style.display = 'inline-block';
        document.getElementById('cloud-status').textContent = '已登录';
        document.getElementById('cloud-status').className = 'cloud-status online';

        // Close modal
        document.getElementById('login-modal').style.display = 'none';

        this.showInAppNotification('云端同步', '登录成功！数据将自动同步', 'success');
    ...
    async syncToCloud() {
        ...
        document.getElementById('cloud-sync').textContent = '同步中...';
        document.getElementById('cloud-status').textContent = '同步中...';
        ...
        try {
            ...
            localStorage.setItem('cloud-backup-' + this.currentUser.id, JSON.stringify(cloudData));
            this.lastSyncTime = new Date().toISOString();

            this.showInAppNotification('云端同步', '数据同步成功', 'success');
            document.getElementById('cloud-status').textContent = '已同步';
            document.getElementById('cloud-sync').textContent = '🔄 同步';
        } catch (error) {
            this.showInAppNotification('云端同步', '同步失败: ' + error.message, 'error');
    ...
    async syncFromCloud() {
        ...
        // Check for cloud backup in localStorage
        const cloudData = localStorage.getItem('cloud-backup-' + this.currentUser.id);
        if (cloudData) {
            try {
                ...
                if (data.tasks && Array.isArray(data.tasks)) {
                    ...
                    this.showInAppNotification('云端同步', '云端数据加载成功', 'success');
                }
            } catch (error) {
